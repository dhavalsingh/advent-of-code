...
def part_b(input)
  ...
  lines.each do |line|
    flag = true
    id_game_split = line.split(':')
    game_id = id_game_split.first[/\d+/].to_i
    games = id_game_split[1].split(';')
    green, blue, red = 0
    min_game = {
      "blue" => 1,
      "green" => 1,
    ...
    games.each do |game|
      matches = game.scan(/(\d+) (\w+)/)
      matches.each do |match|
        if min_game["#{match[1]}"] < match[0].to_i
          min_game["#{match[1]}"] = match[0].to_i
        end
      end
    end
    sum = sum +  (min_game["green"] * min_game["red"] * min_game["blue"])
    #sum = sum + game_id if flag == true
    #puts games
  end
  return sum
end
