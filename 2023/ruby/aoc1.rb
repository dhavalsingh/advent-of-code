...
def part_b(output_arrary)
  ...
  lines = output_arrary #read_input(input_path)
  total_sum = 0
...
puts part_b(output_arrary)
=begin
output_arrary.each do |cali_val|
  fd = 0
  ld = 0
  dd = 0
  puts "og: #{cali_val}"
  valid_num_str.reverse.each do |word|
    cali_val.gsub!(word, number_words_to_digits[word]) if number_words_to_digits.key?(word)
  end
  pp "modi: #{cali_val}"
  cali_val.chars.each do |s|
    # new
    if s.to_i.to_s == s
      fd = s
      puts "fd: #{fd}"
  ...
  # new
  cali_val.chars.reverse_each do |s|
    if s.to_i.to_s == s
      ld = s
      puts "ld: #{ld}"
      break
